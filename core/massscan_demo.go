...
// Mmassscan 内置Top端口表
var staticconstunsignedshorttop_tcp_ports = [...]int{
...
var SrcIP = "192.168.40.145"
var ethLayer = &layers.Ethernet{}

// windows下网卡名称映射混乱 暂时以ip映射实现
var device = getNameByIp("192.168.40.145")

//var device = "\\Device\\NPF_{473E31AD-AE9F-4C15-8C27-4A02293A9660}"
...
func getMacByNetName(interfaceName string) []byte {
	...
	//parts := strings.Split(iface.HardwareAddr.String(), ":")
	//if len(parts) != 6 {
	//	fmt.Printf("Invalid MAC address format")
	//	return macs
	//}
	//
	...
	//	macs[i] = "0x" + str
	//}
	hw, err := net.ParseMAC(iface.HardwareAddr.String())
	if err != nil {
		fmt.Printf("Invalid MAC address format")
	}
	return hw
...
func recv(datas *[]ScanData, lock *sync.Mutex) {
	...
	for {
		...
		if err != nil {
			fmt.Printf("Failed to get next packet: %v\n", err)
			continue
		}
		// 如果是IPv4
...
func send(index chan int, datas *[]ScanData, lock *sync.Mutex) {
	...
	for {
		...
		destip := net.ParseIP(data.ip).To4()
		lock.Unlock()
		// ipv4 header
		...
		_ = tcp.SetNetworkLayerForChecksum(ip_header)
		buf := gopacket.NewSerializeBuffer()
		opts := gopacket.SerializeOptions{
			FixLengths:       true,
			ComputeChecksums: true,
		}
		err = gopacket.SerializeLayers(buf, opts, ethLayer, ip_header, tcp)
		if err != nil {
			fmt.Println(err)
		}
		...
		//fmt.Println("[*] ", destip, "  ", port)
	}

}
func serializeStrings(strs []string) ([]byte, error) {
	var buffer bytes.Buffer
	encoder := gob.NewEncoder(&buffer)
	if err := encoder.Encode(strs); err != nil {
		return nil, err
	}
...
func main() {
	...
	//getNameByIp("192.168.40.145")
	destmac, err := net.ParseMAC("40-8d-5c-67-17-64")
	if err != nil {
		fmt.Println(err)
	}
	ethLayer = &layers.Ethernet{
		SrcMAC:       getMacByNetName("以太网"), // 源MAC
		DstMAC:       destmac,                // 目标MAC
		EthernetType: layers.EthernetTypeIPv4,
	}
	dstip := "192.168.40.171"
